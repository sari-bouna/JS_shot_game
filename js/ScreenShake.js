...
Phaser.Plugin.ScreenShake = function(game, parent){
	...
	*/
	this._moveCamera = function(){
		if (this._settings.shakesMax == 0) this._settings.shakesMax = this._settings.shakesCount;
		
		if (this._settings.shakesCount > 0) {
			var magnitude = ( this._settings.shakesCount / this._settings.shakesMax ) * this._settings.shakesCount;
			var direction = this.game.rnd.integerInRange(-1, +1) * magnitude;

			this.game.camera.x = (this._settings.shakeX ? direction : 0);
			this.game.camera.y = (this._settings.shakeY ? direction : 0);
			
			this._settings.shakesCount--;

			if (this._settings.shakesCount <= 0) {
				this.game.camera.setPosition(0, 0);
				this._settings.shakesMax = 0;
			}
		}
	};
};
